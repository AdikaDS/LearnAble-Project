...
async def get_previous_chips(req, db):
    ...
    # Check both outputContexts and inputContexts
    ...
    # Cek context pilihsubbab-followup -> kembali ke subbab chips (untuk theory handler)
    ...
    # Cek context pilihpelajaran-followup -> kembali ke lesson chips (untuk subbab handler)
    for context in all_contexts:
    ...
    # Cek context pilihjenjang-followup -> kembali ke subject chips (untuk lessons handler)
    for context in all_contexts:
        if "pilihjenjang-followup" in context.get("name", ""):
            ...
            # Fallback: jika tidak ada level, kembali ke level chips
            chips = [
                {"text": "Jenjang SD"},
                {"text": "Jenjang SMP"},
                {"text": "Jenjang SMA"}
