...
def handle_subjects_by_level(level, req):
    ...
    try:
        ...
        if not chips:
            return jsonify({
                "fulfillmentText":
                f"Belum ada pelajaran untuk jenjang {level.upper()}."
            })
        ...
        response = {
            "fulfillmentMessages": [{
                "text": {
                    "text": [
                        f"Berikut pelajaran untuk jenjang {level.upper()} yang tersedia:"
                    ]
                }
            }, {
                "payload": {
        ...
        return jsonify({
            "fulfillmentText":
            "Terjadi kesalahan saat mengambil data pelajaran."
        })
